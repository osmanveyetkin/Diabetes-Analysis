...
def load_clean_data(filepath):
    """CSV dosyasından temizlenmiş veriyi yükler."""
    print(f"[INFO] Yükleniyor: {filepath}")
    return pd.read_csv(filepath)

def train_and_evaluate(X_train, X_test, y_train, y_test):
    """Modelleri eğitir ve tahminleri üretir."""
    print("\n[INFO] Model eğitimi başlatılıyor...")
...
if __name__ == "__main__":
    ...
    # Train/Test Split
    # Train/Test Split
    print("[INFO] Splitting data into Train (80%) and Test (20%)...")
    ...
    print(f"\n[RESULT] Seçilen Model: {best_name}")
    print(f"[INFO] Model kaydediliyor: {MODEL_PATH}")
    
    with open(MODEL_PATH, 'wb') as f:
        pickle.dump(best_model, f)
        
    print("[SUCCESS] Eğitim süreci tamamlandı.")
